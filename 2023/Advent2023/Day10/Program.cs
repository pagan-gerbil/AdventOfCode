namespace Day10
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Starts at {start}.");

            var firstDirection = GetFirstStep(grid, start);
            var firstStep = start;

            switch(firstDirection)
            {
                case Direction.North: firstStep.Row -= 1; break;
                case Direction.South: firstStep.Row += 1; break;
                case Direction.East:  firstStep.Col += 1; break;
                case Direction.West:  firstStep.Col -= 1; break;
            }

            var steps = GetNextStep(grid, firstStep, firstDirection, start);

            Console.WriteLine($"Total steps: {steps}");
            Console.WriteLine($"Furthest Point: {Math.Round(steps / 2.0, 0, MidpointRounding.AwayFromZero)}");
        }

        private static Direction GetFirstStep(char[][] grid, (int Row, int Col) start)
        {
            if (start.Row > 0 && _norths.Contains(grid[start.Row - 1][start.Col]))
            {
                return Direction.North;
            }

            if (start.Row < grid.Length - 1 && _souths.Contains(grid[start.Row + 1][start.Col]))
            {
                return Direction.South;
            }

            if (start.Col > 0 && _wests.Contains(grid[start.Row][start.Col - 1]))
            {
                return Direction.West;
            }

            if (start.Col < grid[0].Length - 1 && _easts.Contains(grid[start.Row][start.Col + 1]))
            {
        ...
        private static int GetNextStep(char[][] grid, (int Row, int Col) here, Direction lastDirection, (int Row, int Col) endPoint)
        {
            var steps = 1;

            var loopLocations = new List<(int Row, int Col)>();
            loopLocations.Add(endPoint);

            while (here != endPoint)
            {
                loopLocations.Add(here);
                switch (grid[here.Row][here.Col])
                {
                    case '|':
                        if (lastDirection == Direction.North) { here.Row -= 1; lastDirection = Direction.North; }
                        else { here.Row += 1; lastDirection = Direction.South; }
                        break;
                    case '7':
                        if (lastDirection == Direction.North) { here.Col -= 1; lastDirection = Direction.West; }
                        else { here.Row += 1; lastDirection = Direction.South; }
                        break;
                    case 'F':
                        if (lastDirection == Direction.North) { here.Col += 1; lastDirection = Direction.East; }
                        else { here.Row += 1; lastDirection = Direction.South; }
                        break;
                    case 'J':
                        if (lastDirection == Direction.South) { here.Col -= 1; lastDirection = Direction.West; }
                        else { here.Row -= 1; lastDirection = Direction.North; }
                        break;
                    case 'L':
                        if (lastDirection == Direction.South) { here.Col += 1; lastDirection = Direction.East; }
                        else { here.Row -= 1; lastDirection = Direction.North; }
                        break;
                    case '-':
                        if (lastDirection == Direction.East) { here.Col += 1; lastDirection = Direction.East; }
                        else { here.Col -= 1; lastDirection = Direction.West; }
                        break;
                    default:
                        break;
                }
            ...
            //FindTilesInsideLoop(grid, loopLocations);
            FloodFill(grid, loopLocations);

            return steps;
        }

        private static void FloodFill(char[][] grid, List<(int Row, int Col)> loopLocations)
        {
            var outsideTiles = new List<(int Row, int Col)>();

            // edge tiles are outside
            for (var i = 0; i < grid.Length; i++)
            {
                if (!loopLocations.Contains((0, i))) outsideTiles.Add((0, i));
                if (!loopLocations.Contains((i, 0))) outsideTiles.Add((i, 0));
                if (!loopLocations.Contains((grid.Length-1, i))) outsideTiles.Add((grid.Length-1, i));
                if (!loopLocations.Contains((i, grid.Length-1))) outsideTiles.Add((i, grid.Length-1));
            }

            var nextLooks = new Stack<(int Row, int Col)>(outsideTiles);

            while (nextLooks.Any())
            {
                var o = nextLooks.Pop();
                if (o.Row > 1) // look north
                {
                    var here = (o.Row - 1, o.Col);
                    if (!loopLocations.Contains(here) && !outsideTiles.Contains(here))
                    {
                        outsideTiles.Add(here);
                        nextLooks.Push(here);
                    }
                }
                if (o.Row < grid.Length - 1) // look south
                {
                    var here = (o.Row + 1, o.Col);
                    if (!loopLocations.Contains(here) && !outsideTiles.Contains(here))
                    {
                ...
                if (o.Col > 1) // look west
                {
                    var here = (o.Row, o.Col - 1);
                    if (!loopLocations.Contains(here) && !outsideTiles.Contains(here))
                    {
                        outsideTiles.Add(here);
                        nextLooks.Push(here);
                ...
                if (o.Col < grid[0].Length - 1) // look east
                {
                    var here = (o.Row, o.Col + 1);
                    if (!loopLocations.Contains(here) && !outsideTiles.Contains(here))
                    {
            ...
            Draw(grid, loopLocations, outsideTiles);

            Console.WriteLine($"There are {loopLocations.Count} loop locations, {outsideTiles.Distinct().Count()} outside tiles and {(grid.Length * grid[0].Length) - loopLocations.Count - outsideTiles.Distinct().Count()} other.");
        }

        private static void Draw(char[][] grid, List<(int Row, int Col)> loopLocations, IEnumerable<(int Row, int Col)> results)
        {
            ...
            foreach (var c in loopLocations)
            {
                grid[c.Row][c.Col] = '#';
            }
        ...
        private static List<(int Row, int Col)> Look(char[][] grid, List<(int Row, int Col)> loopLocations, bool forwards, bool horizontal)
        {
            var insideLoop = new List<(int Row, int Col)>();

            var start = forwards ? 0 : grid.Length - 1;
            var end = forwards ? grid.Length : 0;

            var increment = forwards ? 1 : -1;

            var lessThan = (int s, int e) => { return s < e; };
            var greaterThan = (int s, int e) => { return s >= e; };

            var function = forwards ? lessThan : greaterThan;

            for (var row = start; function(row, end); row += increment)
            {
                var outside = true;

                for (var col = start; function(col, end); col += increment)
                {
                    (int Row, int Col) here = horizontal ? (row, col) : (col, row);

                    if (loopLocations.Contains(here))
                    {
                        outside = false;
                        continue;
                    }
