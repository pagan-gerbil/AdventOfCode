...
namespace Day10
{
    internal class Program
    {
        ...
        private static void FindTilesInsideLoop(char[][] grid, List<(int Row, int Col)> loopLocations)
        {
            var east = Look(grid, loopLocations, true, true);
            var west = Look(grid, loopLocations, false, true);
            var north = Look(grid, loopLocations, true,  false);
            var south = Look(grid, loopLocations, false, false);

            var results = east.Intersect(west).Intersect(north).Intersect(south);

            Console.WriteLine($"There are {results.Count()} interior tiles");
        }
        ...
        private static List<(int Row, int Col)> Look(char[][] grid, List<(int Row, int Col)> loopLocations, bool forwards, bool horizontal)
        {
            ...
            var start = forwards ? 0 : grid.Length;
            ...
            for (var row = start; function(row, end); row += increment)
            {
                ...
                {
                    var here = horizontal ? (row, col) : (col, row);
